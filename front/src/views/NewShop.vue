    ...
    methods: {
      async submitForm() {
        try {
          ...
          if (!userId) {
            alert("L'utilisateur n'est pas connecté ou l'ID utilisateur est manquant.");
            return;
          }
          this.form.owner = userId;
          ...
          if (data.length > 0) {
            ...
            console.log("posX:", this.form.posX);
            console.log("posY:", this.form.posY);
          } else {
          ...
          const apiResponse = await fetch("http://localhost:3000/api/shop", {
            method: "POST",
            body: formData,
          });
          ...
          if (apiResponse.ok) {
            ...
            console.error("Erreur lors de l'enregistrement :", apiResponse.statusText);
